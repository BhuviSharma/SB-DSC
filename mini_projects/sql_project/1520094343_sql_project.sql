...
SELECT facid, name, membercost, monthlymaintenance FROM country_club.Facilities where membercost < ((20*monthlymaintenance)/100)
...
select firstname, surname from country_club.Members where memid = (select max(memid) from country_club.Members)
...
the member name. */
select distinct concat_ws(' ', m.firstname, m.surname) as membername, f.name as courtname from country_club.Bookings  b
...
-- calculate total investment overall for each month
